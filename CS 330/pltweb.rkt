...
(define (page4)
  (let ([req
    ...
    (if (equal? 
         (cdr 
          (assoc 'answer (url-query (request-uri req)))) "text selected")
        (cdr (assoc 'answer (url-query (request-uri req))))
        (cdr (assoc 'answer (url-query (request-uri req))))
        )
    )
...
(define (page6)
  (let ([req
             ...
             `(html (head (title "Cave’s best release?"))
                    (body
                     ...
                     (form ([action ,k-url])
                           ...
                           (input ([type "radio"]
                                   [name "cave"]
                                   [value "Ketsui kizuna jigoku tachi"]))
                           "Ketsui kizuna jigoku tachi "
                           (br)
                           
                           (input ([type "radio"]
                                   [name "cave"]
                                   [value "Mushihimesama Futari 1.5"]))
    ...
    (if (equal? 
         (cdr 
          (assoc 'cave (url-query (request-uri req)))) "text selected")
        (cdr (assoc 'other (url-query (request-uri req))))
        (cdr (assoc 'cave (url-query (request-uri req))))
        )
    )
  )
...
(define (page7)
  (let ([req 
                    ...
                    (body
                     ...
                     (form ([action ,k-url])
                           (input ([type "radio"]
                                   [checked "true"]
                                   [name "treasure"]
                                   [value "Radiant Silvergun"]))
                           " Radiant Silvergun "
                           (br)
                           
                           (input ([type "radio"]
                                   [name "treasure"]
                                   [value "Sin and Punishment"]))
                           " Sin and Punishment "
                           (br)
    ...
    (if (equal? 
         (cdr 
          (assoc 'treasure (url-query (request-uri req)))) "text selected")
        (cdr (assoc 'other (url-query (request-uri req))))
  ...
  (response/xexpr
   `(html (head (title "Results"))
          (body
           (h1 "Answers!")
           ,(if pg1
                `(p "What is the best controller for shooters?"(br) ,pg1)
                "")
           ,(if pg2
                `(p "What is the best gate?"(br) ,pg2)
                "")
           ,(if pg3
                `(p "Do you prefer horizontal or vertical scrolling?"(br) ,pg3)
                "")
           ,(if pg4
                `(p "What is the best horizontal shooter?"(br) ,pg4)
                "")
           ,(if pg5 
                `(p "Which is the better developer?"(br) ,pg5)
                "")
           ,(if pg6
                `(p "What is Cave’s best release?"(br) ,pg6)
                "")
           ,(if pg7
                `(p "What is Treasure’s best release?"(br) ,pg7)
                "")
           )
          )
   )
...
(define (start initial-request)
  ...
  (define response2 
    (if (equal? response1 "Arcade Stick")
        (page2)
        #f))
  ...
  (define response4 
    (if(equal? response3 "Horizantal")
       (page4)
       #f))
  
  (define response5 
    (if(equal? response3 "Vertical")
       (page5)
       #f))
  (define response6 
    (if(equal? response5 "Cave")
       (page6)
       #f))
  
  (define response7 
    (if(equal? response5 "Treasure")
       (page7)
       #f))
